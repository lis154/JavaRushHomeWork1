...
public class Solution {
    ...
    public static byte[] getNetAddress(byte[] ip, byte[] mask) {
        ...
        byte[] newAddres = new byte[4];
        for (int i = 0; i < 4; i++)
        {
            newAddres[i] = (byte) (ip[i] & mask[i]);
        }
        return newAddres;
    }
    ...
    public static void print(byte[] bytes) {
        ...
        {
            byteCur = Integer.toBinaryString(256 + (int)bytes2);
            System.out.println(byteCur.substring(byteCur.length()-8) + " ");
        }
